...
B2=1;
B3=1;
Cn=0.6; % courant number (to calculate dt using stability criterion)
...
while t<=Tmax
    ...
    for i=4:Nx-3
        ...
        hp(i)= h(i)+ dt/(6*dx)*(u(i+2)*h(i+2)-8*u(i+1)*h(i+1)+7*u(i)*h(i));
        up(i)= (1/hp(i))*(u(i)*h(i)+ dt/(6*dx)*(T(i+2)-8*T(i+1)+7*T(i))+dt*g*h(i)*(So-Sf));
    end
    ...
    for i=4:Nx-3
        ...
        hc(i)= hp(i)+dt/(6*dx)*(-up(i-2)*hp(i-2)+8*up(i-1)*hp(i-1)-7*up(i)*hp(i));
        uc(i)= (1/hc(i))*(up(i)*hp(i)+ dt/(6*dx)*(-T(i-2)+8*T(i-1)-7*T(i))+dt*g*hp(i)*(So-Sf));
    end
